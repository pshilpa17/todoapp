document.addEventListener('DOMContentLoaded', function () {
    fetch('http://localhost:6010/getAll')
...
//     fetch('http://localhost:6010/search/' + searchValue)
//     .then(response => response.json())
//     .then(data => loadHTMLTable(data['data']));
// }

function deleteRowById(id) {
    fetch('http://localhost:6010/delete/' + id, {
        method: 'DELETE'
    })
    .then(response => response.json())
    .then(data => {
...
updateBtn.onclick = function() {
    ...
    fetch('http://localhost:6010/update', {
        method: 'PATCH',
        headers: {
...
addBtn.onclick = function () {
    ...
    fetch('http://localhost:6010/insert', {
        headers: {
            'Content-type': 'application/json'
        },
        method: 'POST',
...
function loadHTMLTable(data) {
    ...
    data.forEach(function ({id, todo, datecreated}) {
        tableHtml += "<tr>";
        tableHtml += `<td>${id}</td>`;
